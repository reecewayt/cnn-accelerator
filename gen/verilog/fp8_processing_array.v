...
reg [7:0] a_slices [0:2-1];
reg [7:0] b_slices [0:2-1];
...
always @(i_b_vector, i_a_vector) begin: shadow_slices
    integer i;
    integer j;
    for (i=0; i<2; i=i+1) begin
        a_slices[i] = i_a_vector[(((i + 1) * 8) - 1)-1:(i * 8)];
    end
    for (j=0; j<2; j=j+1) begin
        b_slices[j] = i_b_vector[(((j + 1) * 8) - 1)-1:(j * 8)];
    end
end
...
always @(posedge clk) begin: pe_done_latch_logic
    ...
    if (i_reset == 1) begin
        ...
        if (i_reset) begin
            ...
            all_done = 1'b1;
            for (i=0; i<(2 * 2); i=i+1) begin
                if (mac_done_signals[i]) begin
                    pe_done_latches[i] <= 1;
                end
                all_done = (all_done && pe_done_latches[i]);
            end
            all_pes_done <= all_done;
            if (i_read_en) begin
                temp = 32'h0;
                for (idx=0; idx<(2 * 2); idx=idx+1) begin
                    high = (((idx + 1) * 8) - 1);
                    low = (idx * 8);
                    temp[high-1:low] = c_outputs[idx];
                end
...
always @(ready_signals[0], ready_signals[1], ready_signals[2], ready_signals[3]) begin: ready_logic
    reg ready_temp;
    integer i;
    ready_temp = 1'b1;
    for (i=0; i<(2 * 2); i=i+1) begin
        ready_temp = (ready_temp && ready_signals[i]);
    end
    o_ready_for_new = ready_temp;
end
